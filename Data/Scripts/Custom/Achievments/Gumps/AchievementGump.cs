...
namespace Scripts.Mythik.Systems.Achievements.Gumps
{
    class AchievementGump : Gump
    {
        ...
        public AchievementGump(Dictionary<int, AchieveData> achieves, int total, int pageNumber = 1, int selectedCategoryId = -1, Dictionary<int, AchieveData> viewerAchieves = null, string targetName = null) : base(25, 25)
        {
            ...
            AchievementSystem.AchievementCategory selectedCategory = null;
            for (int si = 0; si < AchievementSystem.Categories.Count; si++)
            {
                if (AchievementSystem.Categories[si].ID == selectedCategoryId)
                {
                    selectedCategory = AchievementSystem.Categories[si];
                    break;
                }
            }

            var categoryList = new List<AchievementSystem.AchievementCategory>();
            if (selectedCategory != null)
            {
                for (int ci = 0; ci < AchievementSystem.Categories.Count; ci++)
                {
                    var cat = AchievementSystem.Categories[ci];
                    if (cat == selectedCategory
                        || cat.ID == selectedCategory.Parent
                        || (0 < cat.Parent && cat.Parent == selectedCategory.ID)
                        || (0 < cat.Parent && cat.Parent == selectedCategory.Parent))
                    {
            ...
            var achievementsList = new List<BaseAchievement>();
            if (selectedCategory != null)
            {
                for (int ai = 0; ai < AchievementSystem.Achievements.Count; ai++)
                {
                    var ac = AchievementSystem.Achievements[ai];
                    bool visible = !ac.HiddenTillComplete || (achieves.ContainsKey(ac.ID) && achieves[ac.ID].IsComplete);
                    bool prereqOk = ac.PreReq == null || (achieves.ContainsKey(ac.PreReq.ID) && achieves[ac.PreReq.ID].IsComplete);

                    if (ac.CategoryID == selectedCategoryId && visible && prereqOk)
            ...
            else
            {
                ...
                {
                    var ac = AchievementSystem.Achievements[ai];
                    if (achieves.ContainsKey(ac.ID) && achieves[ac.ID].IsComplete)
                        achievementsList.Add(ac);
                }
                ...
                achievementsList.Sort((a, b) => {
                    AchieveData da = null;
                    AchieveData db = null;
                    achieves.TryGetValue(a.ID, out da);
                    achieves.TryGetValue(b.ID, out db);
                    if (da == null && db == null) return 0;
                    if (da == null) return 1;
                    if (db == null) return -1;
                    return db.CompletedOn.CompareTo(da.CompletedOn);
                });
            }

            if (m_isOtherView)
            {
                var filtered = new List<BaseAchievement>();
                for (int fi = 0; fi < achievementsList.Count; fi++)
                {
                    var ac = achievementsList[fi];
                    if (achieves.ContainsKey(ac.ID) && achieves[ac.ID].IsComplete)
                        filtered.Add(ac);
                }
                achievementsList = filtered;
            }

            var maxPages = (int)Math.Ceiling((double)achievementsList.Count / ITEMS_PER_PAGE);
            int itemIndex = 0;
            int startIndex = (pageNumber - 1) * ITEMS_PER_PAGE;
            int endIndex = Math.Min(startIndex + ITEMS_PER_PAGE, achievementsList.Count);
            for (int ai = startIndex; ai < endIndex; ai++)
            {
                var achievement = achievementsList[ai];
