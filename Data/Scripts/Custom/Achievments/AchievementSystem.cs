...
namespace Scripts.Mythik.Systems.Achievements
{
	public class AchievementSystem
	{
		...
		private static DiscoveryAchievement AddTown(int achievementId, int categoryId, DiscoverLandAchievement prerequisite, string region)
		{
			...
			var landName = Lands.LandName(prerequisite.Land);
				...
				default:
					{
						...
						int xc;
						int yc;
						location = Worlds.GetTown(0, region, Map.Internal, out place, out xc, out yc);
		...
		private static DiscoveryAchievement AddDungeon(int achievementId, int categoryId, DiscoverLandAchievement prerequisite, string region, bool hasRelic = true)
		{
			...
			// Relic lookup not available on this shard; omit for now
			var relicQuestItem = (string)null;

			string location = null;

			// Try to find dungeon index by name and resolve its location
			int dungeonIndex = FindDungeonIndexByName(region);
			if (dungeonIndex > 0)
			{
				string world;
				Map placer;
				int xc;
				int yc;
				var dungeonName = Worlds.GetDungeonListing(dungeonIndex, out world, out location, out placer, out xc, out yc);
		...
		private static int FindDungeonIndexByName(string name)
		{
			...
			for (int i = 1; i < 1000; i++)
			{
				...
				Map placer;
				int xc, yc;
				var dungeon = Worlds.GetDungeonListing(i, out world, out location, out placer, out xc, out yc);
				if (string.Equals(dungeon, name, StringComparison.OrdinalIgnoreCase))
					return i;
		...
		private static void LoadData()
		{
			Persistence.Deserialize(
				...
				reader =>
				{
					...
					if (version < 1)
					{
							var achievementsLookup = new Dictionary<int, BaseAchievement>();
							foreach (var achievement in Achievements)
							{
								achievementsLookup[achievement.ID] = achievement;
							}

							foreach (var kv in m_featData)
							{
								var id = kv.Key;
								var dataByAchievementId = kv.Value;
								var points = 0;
